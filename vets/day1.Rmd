...
To get a feel for this data we can use the head commmand to look at the first few lines of the file
...
Now lets add a litte text in our plot to show which species are which:
...
If we really wanted to just focus on the fact that setos is smaller another way to do it would be to plot the distribution of petal length in each species.  We can do this by combining the functions `density` and `plot`.
