...
Next they present the results of simulation studies that show that Pagel 1994 [2] approach will return many significant results for both structures like C and D. In fact all simulations of Darwin's scenario (C) result in highly significant results using Pagel’s test.

The problem here is pseudoreplication.  The amount of shared branch length largely determines the significance of the result rather than the number of independent occurences.

So how can we move forward - in addition to looking at our trees and seeing what going on.

#The pseudo and real code for calculating the likelihood of a discrete trait
