...
(1)  Begin coding a pruning algorithm to calculate the likelihood of a discrete trait on a phylogeny.  Start with pseudo-code, moving on to real code if you want.  Even with pseudo-code, do think about data structures (how to represent the tree, variables for calculation bookkeeppiinngg) and how to traverse the tree.  A functioning program is not expected!!  It will be useful to discuss strategies before getting too deep into any implementation.

(2)  Think about new ideas for a phylogenetic model of correlated trait evolution.  Moving beyond Pagel's model of dependent rates, how else can we express the biological idea of two traits evolving together?  We didn't discuss the Maddison & FitzJohn paper much, but my take is that we need to find mathematical/statistical models that align better with the biological view of correlation.  So I think it would be fun and potentially extremely useful to argue through what those could be, beginning at the verbal/pictoral level.

(3) Heath will reread "Is your phylogeny informative"[5] to try and see if there might be a way to apply that approach to our current problems
