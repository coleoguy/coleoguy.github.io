One of the Goldberg lab project for the semester is to make sure we all have a solid understanding of the nuts and bolts of comparative methods.  We started this journey by digging into models of discrete trait evolution.  I think one of the best presentations that I've seen of this was John Huelsenbeck's lecture at the [Bodega Phylogenetics workshop](http://treethinkers.org/).  He also has a [great tutorial](https://molevol.mbl.edu/images/3/37/WoodsHoleHandout.pdf) available online for the Woodshole course which is very similar.  This post is the result of R markdown document that I have created in an attempt to make sure that I understand all of this.  The entire R markdown document is available via github.
...
matrix we have the ending state.  So in this case the rate of transtion 
from state 0 to state 1 is 0.3 while the rate of transition from state 1 to state 0 is .4.  These rates will usually be reffered to as q01 and q10 respectively.  The diagonal of the matrix is set so each row is equal to 0.

What these rates actually descirbe is the exponentially distributed expected waiting time till we see a transition in our trait.  For instance we could use this formula to calculate the waiting time for a transition from state 0 to state 1.
...
using the Q-matrix.  For instance lets say we have a branch in our tree that is 1 unit long and we want to know the probability of experencing transitions between our states given a branch length of 1.  The code below will perform this simulation 10000 times to calculate that probability.
```{r}
...
This means that given that we start in state 0 we have approximately 22% chance of ending in state 1.  This would be tedious and annoying to have to do for every length of branch in a tree and fortunately is not necessary.  Instead we can matrix exponenitiation to calculate the probability matrix for any branch length needed.
```{r}
# we will use the package ape which includes a matrix exponentiation function
library(ape)
matexpo(qmat*1)
```
Here we see that although the Monte Carlo simulation approach is laborious it does indeed produce very similar results to the analytical solution provided by matrix exponentiation.
...
This brings us to a point where we no longer have any node where both daugther branches are tips.  So our next step is to combine the bottom two boxes with one another to set up a pair of probabilites just before the node indicated in red below. Since the evolution in any branch is independent of all other branches when we will combine these by multiplication
...
Conceptually we are now in a simalar position as we were at the begining, but we now have a tree with three tips (the tip at the top plus the green and red node).  So now we simply work our way down the tree till we have a single box at the root of the tree.
...
for(i in 1:length(tip)){
  ...
  text(.9+.7, 2.5, labels=round(.035*.234, digits=4), pos=2,cex=.35)
  ...
  text(0+.2, 2.5, labels=round(pmat1[1,1] * .0942 + pmat1[1,2] * .0082, digits=3), pos=2,cex=.5)
  text(0+.7, 2.5, labels=round(pmat1[2,1] * .0942 + pmat1[2,2] * .0082, digits=3), pos=2,cex=.5)

 
   rect(.0-.3, 3.75-.2, .0+.6, 3.75+.26, col="white", border = "blue")
   lines(x=c(0+.17, 0+.17), y=c(3.75-.2, 3.75+.26), col="blue", lty=3)
  text(0+.2, 3.75, labels=round(.416*.076, digits=3), pos=2,cex=.5)
  text(0+.7, 3.75, labels=round(.446*.033, digits=3), pos=2,cex=.5)
```

So now we have to decide how to sum over the two probabilities at the bottom of the tree into a single likelihood for the data given our Q-matrix, tree, and observed species.  There are a number of ways to do this we could simply take the average of these giving each equal weight.  Or we could weight them by the stationary distribution.  Here we will use an approach developed by Rich Fitzjohn that is presented appendix 1 of this  [paper](http://sysbio.oxfordjournals.org/content/58/6/595.short).  Here we treat the root state as a nuisence parameter and weight the probabilities by how well they explain the data.  In our case this yields the following equation for the final likelihood:

$0.032\frac{0.032}{.032+.015}+0.015\frac{0.015}{.032+.015}=3.62$
...
So we know that we can step by step calculate the likelihood of a trait.  If this were DNA sequences then we could simply repeat this process for each site and take the product of all of the site likelihoods.  However, this is laborious and what we want is to make a useful function that takes th tree, data, and the qmatrix and returns the likelihood.

#Code to calculate the likelihood of discrete data
