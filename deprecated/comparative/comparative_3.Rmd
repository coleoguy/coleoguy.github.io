---
title: "Study of comparative methods -- Simulating data for pseudoreplication problem."
...
___
#simulated data - Darwin's scenario states 
```{r}
...
___
#simulated data - unreplicated burst states 
...
___
#simulated data - perfectly replicated evolution states 
...
___
#simulated data - dependant evolution states 
...
___
#simulated data - independant evolution states 

```{r}
independant.data1 <- independant.data2 <- list()
for(i in 1:100){
  current.tree <- set.phy[[i]]
  independant.data1[[i]] <- diversitree::sim.character(current.tree, c(.5,.5), x0=0, model="mk2")
  independant.data2[[i]] <- diversitree::sim.character(current.tree, c(.5,.5), x0=0, model="mk2")
}
```
...
rm(Q, counter, current.tree, data.good, descendants, foo, i, j, max.node, min.node, picked, picked.nodes, set.state, states,states1,states2,tip.cols, tt)
```
