...
summary_lines <- c(sprintf("Cool - Warm mean difference = %.2f mm", difference),
 ...
 sprintf("Welch t = %.2f; df = %.2f; p = %.4g", fit$statistic, fit$parameter, fit$p.value))
report <- sprintf("Mean body length differed by %.2f mm (Cool minus Warm; 95%% CI %.2f to %.2f). Welch's t = %.2f, df = %.2f, p = %.4g. Group ordering determines the sign; unequal group variances do not require switching to ranks.", difference, fit$conf.int[1], fit$conf.int[2], fit$statistic, fit$parameter, fit$p.value)
