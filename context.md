...
- **CUREs Karyotype Database** (`cures-karyotype-database.html`): Brought into site design system — replaced custom header with standard site header/nav, added site footer with scroll-top, loaded `main.css` + Bootstrap + Bootstrap Icons, added SEO meta tags (description, OG, canonical), favicon. Functional CSS scoped under `.db-app` and `.db-container` to avoid conflicts with `main.css`. Added to `sitemap.xml`. **v1.1 (Apr 2026)**: Added per-row citation column sourced from the "new tracking sheet" Google Sheet (`1h0rwG3tZN9HbcO35NXhuQBumSj3yEZG6fESqXxe_bSQ`, Karyotype Source column). Injected `const CITATIONS` lookup (56 clade→citation entries) next to DATA. Table has 4th column "Citation" (2-line truncated, click to expand). CSV export includes citation. Sort supports citation column. Name-mapping fixups between DATA and sheet: Bryophyta↔Bryophytes, Cetacea↔Cetaceans, Gymnospermae↔Gymnosperms, Pteridophyta↔Pteridophytes, Serpentes↔Serprentes (sheet typo). Added static CSV at `data/cures-karyotype-database.csv` (63,542 rows × 4 cols, 8 MB) with accompanying `data/README.md`. Page now exposes two download buttons: "Download filtered CSV" (dynamic, current selection) and "Full dataset (CSV)" (static file, direct link).
