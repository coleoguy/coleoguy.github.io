...
for(i in 1:iter){
  null[i, 1] <- sum(sample(1:10, size= dat[3, 11],
                           replace=T, prob=parent.probs)==2)
  null[i, 2] <- sum(sample(1:10, size= dat[4, 11],
                           replace=T, prob=daughter.probs)==2)
}

plot(density(null[,1]))
abline(v = dat[3, 2], col="red")
sum(null[,1] >= dat[3, 2])/iter

plot(density(null[,2]))
abline(v = dat[4, 2], col="red")
sum(null[,2] >= dat[4, 2])/iter
